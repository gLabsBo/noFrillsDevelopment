...
while ~feof(f)                          % and read until it ends
    ...
    if ( s(1)=='[' ) && (s(end)==']' )
        ...
        [par,val] = strtok(s, '=');
        val = CleanValue(val, delimiter);
        val = tryUndersdandTheDataType(val);
...
function res = CleanValue(s,delimiter)
res = strtrim(s);
if strcmpi(res(1),delimiter)>0
    res=res(2:end);
end
res = strtrim(res);
end

function val = tryUndersdandTheDataType(val)
    if strcmp('[]',[val(1),val(end)])>0
        %Presumo si tratti di una matrice
        val=strtrim(val);
        if ismember(',',val)
            arraySeparator = ',';
        else
        ...
        val = val(2:end-1);
        val = strsplit(val,arraySeparator);
        valOut=zeros(size(val));
        for t = 1:numel(val)
            valOut(t)=isDataBool(val{t});
            valOut(t)=isDataNumber(val{t});
        end
        val=valOut;
    else
        val=isDataBool(val);
        val=isDataNumber(val);
    end
end

function val = isDataBool(val)
    if strcmpi('true',val)>0
        val = true;
    elseif strcmpi('false',val)>0
        val = false;
    end
    
end

function val= isDataNumber(val)
    isNum=false;
    try 
