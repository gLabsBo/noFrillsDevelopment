...
function temp = XYZtoCorColorTemp(xyz)
    uvt = getUVT();
    rt = getRT();
    if xyz(1) < 1.0e-20 && xyz(2) < 1.0e-20 && xyz(3) < 1.0e-20
    	temp = -1;     % protect against possible divide-by-zero failure */
        return;
    end
    us = (4.0 * xyz(1)) / (xyz(1) + 15.0 * xyz(2) + 3.0 * xyz(3));
    vs = (6.0 * xyz(1)) / (xyz(1) + 15.0 * xyz(2) + 3.0 * xyz(3));
    dm = 0.0;
    for i = 1:31 
    ...
    if i == 31
        temp = -1;     % bad XYZ input, color temp would be less than minimum of 1666.7 degrees, or too far towards blue */
        return;
    end
...
rt = [ ...       
         realmin,  10.0e-6,  20.0e-6,  30.0e-6,  40.0e-6,  50.0e-6, ...
         60.0e-6,  70.0e-6,  80.0e-6,  90.0e-6, 100.0e-6, 125.0e-6, ...
        150.0e-6, 175.0e-6, 200.0e-6, 225.0e-6, 250.0e-6, 275.0e-6, ...
        300.0e-6, 325.0e-6, 350.0e-6, 375.0e-6, 400.0e-6, 425.0e-6, ...
        450.0e-6, 475.0e-6, 500.0e-6, 525.0e-6, 550.0e-6, 575.0e-6, ...
