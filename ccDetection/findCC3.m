function [Clin,X,tt]=findCC3(t,I,iLinear,angNum,perspNum,silentMode, fileName, visible)
...
if index==1
    ...
    im=I3(max(range1(1),1):min(range1(end),size(I3,1)),max(range2(1),1):min(range2(end),size(I3,2)));
...
%     colorCLinear=colorCLinear(bbox(1):bbox(2),bbox(3):bbox(4),:);
    ...
    [RGBlin, RGBwb, X] = visualizePatches2(K,colorC,kLinear, px,py,relx,rely,ang,center, imageOut, visible);
    ...
    [RGBlin, RGBwb, X] = visualizePatches(K,colorC,kLinear, px,py,relx,rely,ang,center, imageOut, visible);
    Clin = RGBlin;
end
